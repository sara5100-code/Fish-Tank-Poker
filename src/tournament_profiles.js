...
const PREFLOP_RANGE_CHARTS={
  ...
  vs3bet:{
    deep100:{pure:'QQ+,AKs,AKo,AQs,JJ',mix:'TT-99,AQo,AJs,KQs'},
    mid50:{pure:'QQ+,AKs,AKo',mix:'JJ,TT,AQs,AQo'},
    short20:{pure:'JJ+,AKs,AKo,AQs',mix:'TT,99,AQo,AJs,KQs'},
    push14:{pure:'TT+,AQs+,AKo',mix:'99-77,AJs-ATs,KQs,AQo'}
  },
  vs4bet:{
    deep100:{pure:'KK+,AKs',mix:'QQ,AKo'},
    mid50:{pure:'KK+,AKs,AKo',mix:'QQ'},
    short20:{pure:'QQ+,AKs,AKo',mix:'JJ,AQs'},
    push14:{pure:'JJ+,AKs,AKo,AQs',mix:'TT,AQo,AJs,KQs'}
  }
};
...
function preflopStackBucket(stackBB){
  ...
  if(bb<=16)return'push14';
  if(bb<=25)return'short20';
  if(bb<=65)return'mid50';
  return'deep100';
}
function preflopChartLookup(kind,ht,pos,totalP,opts){
  ...
  let actionJP=kind==='open'?'Open':kind==='threeBet'?'3bet':kind==='vs3bet'?'Call/4bet':kind==='vs4bet'?'Call/5bet':(pos==='BB'?'Call/defend':'Call');
  let mix=status==='pure'
