...
function naturalRangeActionUpdateText(ev){
  ...
  // [Codex fix 2026-08-02] レンジ更新を内部ラベルではなく「相手に何が残ったか」で説明する。
  const density=(p.valueDensityPct!=null&&p.bluffCandidatePct!=null)
    ?'公開ライン上は、バリュー候補が約'+p.valueDensityPct+'%、ブラフ候補が約'+p.bluffCandidatePct+'%です。'
    :'';
  const boardNote=completed
    ?(isRiver?'リバーでは未完成ドローはもうありません。相手の完成役、空振りしたドローのブラフ、ブラフキャッチ候補を分けて見ます。':'このボードは完成役や強いドローが残りやすいので、ワンペアだけで大きく膨らませる時は慎重に見ます。')
  ...
  if(p.rangeState==='capped'){
    ...
    reason=isRiver?'相手が複数ストリートで打ってリバーまで来ています。途中で弱いハンドは降りやすいので、最後に残るのは強いバリューと、数の限られたブラフ候補です。ライブ$2/$5では、このラインの大きいベットはブラフ不足になりやすいです。':'相手が複数ストリートで打っています。ストリートが進むほど、弱いワンペアや空振り気味の手は減り、強いバリューと強いドロー寄りに絞られます。ライブ$2/$5では、このラインの大きいベットはブラフ不足になりやすいです。';
  }else if(p.rangeState==='turn_call_dense'){
    reason='ターンでこちらのベットにコールされた時点で、相手から完全な空振りは少し減ります。リバーではペア以上の完成役、完成したドロー、空振りした一部の強いドローやブロッカーが中心です。薄いバリューやブラフは、相手が何でコールしてくれるかを先に考えます。';
  }else{
  ...
  return [reason,boardNote,density,size,action].filter(Boolean).join(' ');
}
