...
const OPPONENT_NOTE_KEY='fish_tank_opponent_notes_v1';
let OPPONENT_READ_ENABLED_FALLBACK=false;
let OPPONENT_NOTES_FALLBACK={};
...
function opponentNoteKeyFor(playerOrName){
  if(!playerOrName)return'unknown';
  if(typeof playerOrName==='string')return playerOrName;
  if(playerOrName.profile&&playerOrName.profile.displayName)return playerOrName.profile.displayName;
  return playerOrName.name||'unknown';
}
function opponentNotes(){
  const fallback=Object.assign({},OPPONENT_NOTES_FALLBACK);
  try{
    const raw=localStorage.getItem(OPPONENT_NOTE_KEY);
    const parsed=raw?JSON.parse(raw):{};
    if(parsed&&typeof parsed==='object')return Object.assign(fallback,parsed);
    return fallback;
  }catch(e){return fallback;}
}
function opponentNoteText(playerOrName){
  const notes=opponentNotes();
  return notes[opponentNoteKeyFor(playerOrName)]||'';
}
function setOpponentNote(playerOrName,value){
  const key=opponentNoteKeyFor(playerOrName);
  const notes=opponentNotes();
  const txt=String(value||'').slice(0,1200);
  if(txt)notes[key]=txt;
  else delete notes[key];
  OPPONENT_NOTES_FALLBACK=Object.assign({},notes);
  try{localStorage.setItem(OPPONENT_NOTE_KEY,JSON.stringify(notes));}catch(e){}
}
...
function renderOpponentNotesPanel(){
  if(!game)return '<div class="gto-tip opponent-notes-panel"><div class="tip-title">相手ノート</div>ゲーム開始後に相手ごとのメモを表示します。</div>';
  const ais=game.players.filter(function(p){return !p.isHuman&&p.active&&p.profile;});
  if(!ais.length)return '<div class="gto-tip opponent-notes-panel"><div class="tip-title">相手ノート</div>メモ対象のAIがいません。</div>';
  const rows=ais.map(function(p){
    const key=opponentNoteKeyFor(p);
    const ev=opponentReadEvidenceForName(p.name);
    const hint=ev.clues&&ev.clues.length?ev.clues.join(' / '):'まだ材料不足';
    const note=opponentNoteText(p);
    return '<div class="op-note-row"><div class="op-note-head"><div class="op-read-main"><div class="ai-dot" style="background:'+p.profile.color+'"></div><div><div class="ai-row-name" style="color:'+p.profile.color+'">'+escapeHTML(p.profile.displayName)+'</div><div class="op-read-style">'+escapeHTML(hint)+'</div></div></div><span class="op-note-saved" data-op-note-saved="'+escapeHTML(key)+'">保存済み</span></div><textarea class="op-note-input" data-op-note-name="'+escapeHTML(key)+'" placeholder="例: リバー大サイズはかなり強い。小さいベットは薄いバリュー多め。">'+escapeHTML(note)+'</textarea></div>';
  }).join('');
  return '<div class="gto-tip opponent-notes-panel"><div class="tip-title">相手ノート</div><div class="op-read-sub">相手の癖を自分の言葉で残します。同じ表示名のAIが再登場した時にも引き継がれます。</div><div class="op-note-list">'+rows+'</div></div>';
}
...
const LIVE_PRACTICE_GUIDES=[
  {title:'テーブル/シート選択',
   key:'table',
   weak:'ベット目的 / 相手選び',
   practice:{mode:'フロップトレーニング',modeValue:'scenario',focus:'ポストフロップのベット目的',reason:'相手が降りる手と払う手を考えながら、ベット目的を確認します。'},
   text:'勝ちやすさは自分の腕だけで決まりません。深いスタックでルースにコールする相手が多く、強い常連が左に少ない席を優先します。きつい卓で無理に戦うより、良いゲームを選ぶこと自体が大きなEVです。'},
  {title:'ストラドルポット',
   key:'straddle',
   weak:'フロップ前の入口 / SPR',
   practice:{mode:'リングゲーム',modeValue:'normal',focus:'フロップ前の入口整理',reason:'参加する手と降りる手を整理し、後ろから圧を受けても崩れにくい入口を作ります。'},
   text:'ストラドルが入ると実質BBが大きくなり、スタックは浅くなります。プリフロップは少しタイトに、ポストフロップはSPRが下がる前提でワンペアの扱いを決めます。参加するなら、後ろから大きくアイソされても困りにくい手を選びます。'},
  {title:'ティルトの兆候',
   key:'tilt',
   weak:'ティルト / 取り返し癖',
   practice:{mode:'リングゲーム',modeValue:'normal',focus:'主テーマ確認',reason:'一つのテーマだけを意識して、取り返し癖ではなく判断の質を守ります。'},
   text:'取り返したい、相手を懲らしめたい、さっきの負けを理由にコールしたい。この感覚が出たら一度席を離れる合図です。正しい判断を続けられない状態では、良いハンドを待っても利益を守れません。'},
  {title:'セッション終了判断',
   key:'session-end',
   weak:'リバー判断 / 集中切れ',
   practice:{mode:'リングゲーム',modeValue:'normal',focus:'リバー判断',reason:'ワンペアの受けすぎと薄バリューの境目を、集中して反復します。'},
   text:'勝っている時も負けている時も、疲労と集中力を基準にします。ミスが増えた、リバー判断が雑になった、相手のレンジを考えずにボタンを押している。そう感じたら、まだゲームが良くても終了候補です。'},
  {title:'バンクロール',
   key:'bankroll',
   weak:'資金管理 / 無理な着席',
   practice:{mode:'リングゲーム',modeValue:'normal',focus:'主テーマ確認',reason:'無理な着席で判断を崩さないように、通常ハンドで一つの狙いを守ります。'},
  ...
  {title:'チップハンドリングとエチケット',
   key:'etiquette',
   weak:'ライブ所作 / 信頼',
   practice:{mode:'リングゲーム',modeValue:'normal',focus:'主テーマ確認',reason:'落ち着いた操作と判断を両立させるため、通常ハンドでテンポを整えます。'},
   text:'ベット額ははっきり置き、相手のアクション前に余計な反応をしない。ショーダウンでは自分の手を明確に開き、ディーラーや他プレイヤーを急かさない。テーブルで信頼される振る舞いは、長時間プレーするうえで大事な土台です。'}
];
function sessionPracticeDataAttrs(rec){
  rec=rec||{};
  return ' data-mode="'+sessionTextHTML(rec.modeValue||'normal')+'" data-focus="'+sessionTextHTML(rec.focusValue||'')+'" data-preset="'+sessionTextHTML(rec.presetValue||'')+'" data-mode-label="'+sessionTextHTML(rec.mode||'')+'" data-focus-label="'+sessionTextHTML(rec.focus||'')+'" data-status="'+sessionTextHTML(rec.status||'')+'" data-reason="'+sessionTextHTML(rec.reason||'')+'"';
}
...
function sessionPracticeRecommendation(focus,stats){
  ...
  const hist=getSessionFocusHistory();
  const title=sessionFocusTitleText(focus);
  const recent=hist[0]||null;
  const prev=hist[1]||null;
  const sameRecent=recent&&recent.title===title;
  const recovered=sameRecent&&recent.state==='good'&&prev&&prev.title===title&&prev.state==='warn';
  const repeat=sessionFocusRepeatCandidate(hist);
  if(repeat&&repeat.title===title){
    rec.status='一点集中';
    rec.reason='同じテーマが続いています。新しい課題へ広げず、まずこの練習で一点だけ直します。'+rec.reason;
  }else if(recovered){
    rec.status='卒業確認';
    rec.reason='前回の継続課題を今回は達成まで戻せています。この練習は軽い確認用にして、次回は別テーマへ広げる準備をします。'+rec.reason;
  }else if(r&&r.state==='warn'){
    rec.status='継続推奨';
...
function renderSessionPracticeRecommendation(focus,stats){
  ...
  const data=sessionPracticeDataAttrs(rec);
  return '<div class="session-recommend"><div class="session-recommend-title">おすすめ練習'+sessionTextHTML(label)+': '+sessionTextHTML(rec.mode)+' / '+sessionTextHTML(rec.focus)+'</div><div class="session-recommend-body">'+sessionTextHTML(rec.reason)+'</div>'+guide+'<div class="session-recommend-actions"><button type="button" class="session-apply-practice"'+data+'>この練習を設定</button><button type="button" class="session-start-practice"'+data+'>このテーマで開始</button></div></div>';
}
...
function sessionStatsSnapshot(stats){
  ...
  return{
    ...
    profitBB:Number(stats.profitBB)||0,
    profitHands:Number(stats.profitHands)||0,
...
function sessionStatsSinceBaseline(stats,base){
  ...
  return{
    ...
    profitBB:(Number(snap.profitBB)||0)-(Number(base.profitBB)||0),
    profitHands:sub(snap.profitHands,base.profitHands),
    poCount:poCount,
...
function runFishTankRegressionTests(){
  ...
  add('ライブ実戦教材: 教材カードから対応練習を開始できる',function(){
    if(typeof renderLivePractice!=='function'||typeof sessionPracticeDataAttrs!=='function')return false;
    const html=renderLivePractice();
    return /この練習を開始/.test(html)
      &&/live-guide-start/.test(html)
      &&/data-mode="scenario"[^>]*data-focus-label="ポストフロップのベット目的"/.test(html)
      &&/data-mode="normal"[^>]*data-focus-label="リバー判断"/.test(html)
      &&/data-mode="normal"[^>]*data-focus-label="フロップ前の入口整理"/.test(html);
  });
  ...
  add('セッションチェック: おすすめ練習を設定ボタンとして描画する',function(){
    ...
    return /session-apply-practice/.test(bbHtml)
      &&/session-start-practice/.test(bbHtml)
  ...
  add('セッションチェック: 成長ログから次回練習の続行/卒業を決める',function(){
    if(typeof sessionPracticeRecommendation!=='function'||typeof storeSessionFocusHistory!=='function')return false;
    const old=localStorage.getItem(SESSION_FOCUS_HISTORY_KEY);
    const oldFallback=SESSION_FOCUS_HISTORY_FALLBACK;
    const focus={title:'次回の一点: リバーで降りる力',body:'WTSDを締めます。'};
    try{
      storeSessionFocusHistory([
        {title:'リバーで降りる力',state:'good',sample:'前回後: 10ハンド'},
        {title:'リバーで降りる力',state:'warn',sample:'前回後: 10ハンド'}
      ]);
      const recovered=sessionPracticeRecommendation(focus,{hands:12,wtsdSaw:10,wtsdWent:3});
      storeSessionFocusHistory([
        {title:'リバーで降りる力',state:'warn',sample:'前回後: 10ハンド'},
        {title:'リバーで降りる力',state:'improving',sample:'前回後: 10ハンド'}
      ]);
      const repeat=sessionPracticeRecommendation(focus,{hands:12,wtsdSaw:10,wtsdWent:7});
      return recovered&&recovered.status==='卒業確認'
        &&/別テーマへ広げる準備/.test(recovered.reason)
        &&repeat&&repeat.status==='一点集中'
        &&/新しい課題へ広げず/.test(repeat.reason);
    }finally{
  ...
  add('実地監査: サイズ理由つきベット説明の長文化を検出する',function(){
    const long='このベットは対象レンジとサイズ理由を整理します。サイズの理由は、ボードと相手傾向に合わせることです。'.repeat(12);
    const issues=auditIssuesForHand({players:[]},{evals:[{
      street:'flop',
      action:'bet',
      quality:'ok',
      coachComment:long,
      postflopBetPurposeProfile:{lane:'protectionValue',targetPlan:{target:'下のワンペア'}},
      boardTextureSizeProfile:{pct:50,reason:'2トーンはドローに払わせたい一方、弱い手で大きくしすぎないサイズが中心です。'}
    }]});
    return issues.some(function(i){return i.type==='postflop-size-comment-too-long'&&i.severity==='medium';});
  });
  ...
  add('実地監査: 長すぎるリバー受け説明を検出する',function(){
    const long='必要勝率とブラフ候補とブロッカーを確認します。'.repeat(18);
    const issues=auditIssuesForHand({players:[]},{evals:[{
      street:'river',
      action:'call',
      quality:'bad',
      coachComment:long,
      liveCashRiverDecisionProfile:{lane:'riverOnePairCatch',sizePct:75,completed:true,blocker:{label:'フラッシュブロッカーなし'}}
    }]});
    return issues.some(function(i){return i.type==='river-facing-comment-too-long'&&i.severity==='medium';});
  });

  add('コーチ文: リバー受け説明はブロッカー統合後も長文化させない',function(){
    const txt=coachReviewText({
      street:'river',
      action:'call',
      quality:'bad',
    ...
    return txt.length<360&&/必要勝率/.test(txt)&&/ブロッカー面/.test(txt)&&/フォールド寄り/.test(txt);
  });
  ...
  add('実地監査: クリーンseedの監査バッチは修正キューを空に保つ',function(){
    const b=runFishTankAuditBatch({perMode:1,seed:20260804,maxExamples:8,maxActions:70});
    return !!(b&&b.totalHands>=6&&b.criticalCount===0&&b.highCount===0&&b.mediumCount===0&&b.repairQueue&&b.repairQueue.totalIssues===0);
  });

  add('実地監査: AIオープン幅確認後もAIレンジ違和感を出さない',function(){
    const b=runFishTankAuditBatch({perMode:1,seed:20260805,maxExamples:8,maxActions:80});
    const aiIssues=(b.suspiciousHands||[]).some(function(h){
      return (h.issues||[]).some(function(i){return i.type==='ai-preflop-premise';});
    });
    return !!(b&&b.totalHands>=6&&!aiIssues);
  });
  ...
  add('相手ノート: 表示名ごとに保存して再表示できる',function(){
    const old=localStorage.getItem(OPPONENT_NOTE_KEY);
    try{
      localStorage.removeItem(OPPONENT_NOTE_KEY);
      const p=regressionPlayer('seat-a',false,['7c','7d'],{chips:500,profile:AI_PROFILES.yu});
      setOpponentNote(p,'リバー大サイズは強め。');
      const p2=regressionPlayer('別席',false,['8c','8d'],{chips:500,profile:AI_PROFILES.yu});
      return opponentNoteText(p2)==='リバー大サイズは強め。';
    }finally{
      if(old==null)localStorage.removeItem(OPPONENT_NOTE_KEY);
      else localStorage.setItem(OPPONENT_NOTE_KEY,old);
    }
  });

  add('相手ノート: ノートパネルに保存済みメモを描画する',function(){
    const oldGame=game;
    const old=localStorage.getItem(OPPONENT_NOTE_KEY);
    try{
      localStorage.removeItem(OPPONENT_NOTE_KEY);
      const p=regressionPlayer('seat-a',false,['7c','7d'],{chips:500,profile:AI_PROFILES.yu});
      game={players:[regressionPlayer('あなた',true,['As','Ad'],{chips:500}),p],handHistory:[]};
      setOpponentNote(p,'小さいベットは薄め。');
      const html=renderOpponentNotesPanel();
      return /相手ノート/.test(html)&&/yu/.test(html)&&/小さいベットは薄め。/.test(html)&&/data-op-note-name="yu"/.test(html);
    }finally{
      game=oldGame;
      if(old==null)localStorage.removeItem(OPPONENT_NOTE_KEY);
      else localStorage.setItem(OPPONENT_NOTE_KEY,old);
  ...
  add('相手ノート: サイドとボトムシートにノートタブを持つ',function(){
  ...
  add('収支/品質分離: 固定ハンド列でbb/100を計算する',function(){
    const oldStats=sessionStats;
    try{
      sessionStats=normalizeProfitStats(Object.assign({},_statsDef,{hands:0,scores:[],pfScores:[],poScores:[],byPos:{},cbet:{opp:0,bet:0},fToCbet:{opp:0,fold:0},lessonStats:emptyLessonStats(),profitBB:0,profitHands:0,profitByHand:[]}));
      const h1Players=[
        regressionPlayer('あなた',true,['As','Ah'],{chips:1050,totalInvested:30}),
        regressionPlayer('villain',false,['Ks','Kh'],{chips:950,totalInvested:50})
      ];
      const h1=regressionHand({players:h1Players,bigBlind:10,winners:[{player:h1Players[0],playerIdx:0,amount:80,byFold:false}],decisions:[],pot:80});
      const h2Players=[
        regressionPlayer('あなた',true,['Qd','Js'],{chips:960,totalInvested:40,folded:true}),
        regressionPlayer('villain',false,['9s','9h'],{chips:1040,totalInvested:40})
      ];
      const h2=regressionHand({players:h2Players,bigBlind:10,winners:[{player:h2Players[1],playerIdx:1,amount:80,byFold:true}],decisions:[],pot:80});
      updateSessionStats(h1,{score:90,evals:[]});
      updateSessionStats(h2,{score:90,evals:[]});
      return sessionStats.profitHands===2&&Math.round(sessionStats.profitBB*10)/10===1&&Math.round(sessionStatsBb100(sessionStats))===50;
    }finally{
      sessionStats=oldStats;
    }
  });

  add('収支/品質分離: 傾向タブに実収支と判断品質を並べる',function(){
    const oldStats=sessionStats;
    try{
      sessionStats=normalizeProfitStats(Object.assign({},_statsDef,{hands:6,scores:[82,84,86,83,85,84],pfScores:[80,82],poScores:[84,86],byPos:{},cbet:{opp:0,bet:0},fToCbet:{opp:0,fold:0},lessonStats:emptyLessonStats(),profitBB:-3,profitHands:6,profitByHand:[-1,-1,0,0,-1,0]}));
      const html=renderQualityMoneyBand(sessionStats,84);
      return /判断品質/.test(html)&&/実収支/.test(html)&&/bb\/100/.test(html)&&/短期の収支は分散/.test(html);
    }finally{
      sessionStats=oldStats;
    }
  });

  add('プリフロップドリル: UTG K2o openはフォールド正解にする',function(){
    const spot=preflopDrillBuildSpot({base:PREFLOP_DRILL_SPOTS[0],pos:'UTG',ht:'K2o',totalP:6,stackBB:100});
    const fold=preflopDrillEvaluateSpot(spot,'fold');
    const raise=preflopDrillEvaluateSpot(spot,'raise');
  ...
  add('プリフロップドリル: vs3BETの継続候補はCall/Raiseを許容する',function(){
    ...
    const raise=preflopDrillEvaluateSpot(spot,'raise');
    const fold=preflopDrillEvaluateSpot(spot,'fold');
    return call.ok&&raise.ok&&!fold.ok&&/vs3BET|3BET|Call\/4bet/i.test(call.text+call.mix);
  });

  add('プリフロップドリル: ミスを弱点統計へ流し込める',function(){
    const oldStats=sessionStats;
    const oldState=preflopDrillState;
    try{
      sessionStats=Object.assign({},_statsDef,{hands:0,scores:[],pfScores:[],poScores:[],byPos:{},cbet:{opp:0,bet:0},fToCbet:{opp:0,fold:0},lessonStats:emptyLessonStats()});
      const spot=preflopDrillBuildSpot({base:PREFLOP_DRILL_SPOTS[0],pos:'UTG',ht:'K2o',totalP:6,stackBB:100});
      const ev=preflopDrillEvaluateSpot(spot,'raise');
      preflopDrillState={index:1,correct:0,answers:[{spot:spot,action:'raise',result:ev}],current:spot,finished:false};
      finishPreflopDrillSession();
      const c=sessionStats.lessonStats.categories['preflop-entry'];
      return !!(c&&c.count===1&&c.bad===1);
    }finally{
      sessionStats=oldStats;
      preflopDrillState=oldState;
    }
  });
...
function auditIssuesForHand(hr,an,label){
  ...
  (an.evals||[]).forEach(function(e){
    ...
    if(e.postflopBetPurposeProfile&&e.boardTextureSizeProfile&&coach.length>380){
      // [Codex fix 2026-08-04] サイズ理由を本文へ足した後も、ベット説明が教材として読める長さに収まるか監査する。
      addAuditIssue(issues,'medium','postflop-size-comment-too-long',street+' '+action+' のベットサイズ説明が長く、対象レンジ・サイズ理由・結論が埋もれる可能性があります。',{length:coach.length,betPurpose:e.postflopBetPurposeProfile,sizeProfile:e.boardTextureSizeProfile,text:coach});
    }
    if(e.liveCashRiverDecisionProfile&&/^river(OnePairCatch|DisciplineFold|RaiseResponse)/.test(e.liveCashRiverDecisionProfile.lane||'')&&coach.length>360){
      // [Codex fix 2026-08-03] 必要勝率・ブラフ量・ブロッカーを足したリバー受け説明が長文化していないか監査する。
      addAuditIssue(issues,'medium','river-facing-comment-too-long',street+' '+action+' のリバー受け説明が長く、必要勝率・ブラフ量・ブロッカーの要点が埋もれる可能性があります。',{length:coach.length,riverProfile:e.liveCashRiverDecisionProfile,text:coach});
    }
...
function auditIssuePriorityWeight(issue){
  ...
  const type={
    ...
    'postflop-size-comment-too-long':59,
    'river-facing-comment-too-long':62,
...
function auditRepairSuggestion(type){
  const map={
    ...
    'postflop-size-comment-too-long':'ベット説明を「対象レンジ」「今回サイズ/目安」「サイズ理由1文」に圧縮する。相手タイプとボード理由を重複させない。',
    'river-facing-comment-too-long':'リバー受け説明を「必要勝率」「ブラフ量」「ブロッカー有無」「結論」の4点に圧縮する。',
...
// ============================================================
// [Codex fix 2026-07-31] Phase 4-3: プリフロップ即答ドリル
// ============================================================
var _preflopDrillMode=false;
const PREFLOP_DRILL_TOTAL=20;
let preflopDrillState=null;
const PREFLOP_DRILL_SPOTS=[
  {type:'open',label:'先入れオープン',kind:'open',positions:['UTG','MP','CO','BTN','SB'],openerPos:''},
  {type:'flat',label:'オープンへの対応',kind:'flat',positions:['CO','BTN','SB','BB'],openerPos:'CO'},
  {type:'vs3bet',label:'自分のオープン後に3BETを受けた場面',kind:'vs3bet',positions:['CO','BTN'],openerPos:'BB'},
  {type:'squeeze',label:'スクイーズ候補',kind:'threeBet',positions:['BTN','SB','BB'],openerPos:'CO',caller:true}
];
function preflopDrillCardCode(rank,suit){return new Card(rank,suit);}
function preflopDrillHandFromType(ht){
  ht=ht||'AKo';
  if(ht.length===2)return[preflopDrillCardCode(ht[0],'s'),preflopDrillCardCode(ht[1],'h')];
  const r1=ht[0],r2=ht[1],suited=ht[2]==='s';
  return[preflopDrillCardCode(r1,'s'),preflopDrillCardCode(r2,suited?'s':'h')];
}
function preflopDrillPickHandType(){
  const pool=HAND_RANK_169||['AA','AKs','AQo','KQs','77','A5s','KJo','T9s','72o'];
  return pool[Math.floor(Math.random()*pool.length)]||'AKo';
}
function preflopDrillActionLabel(a){return a==='raise'?'レイズ':a==='call'?'コール':'フォールド';}
function preflopDrillPrimaryAction(spot,chart){
  if(!chart||chart.status==='out')return'fold';
  if(spot.kind==='open'||spot.kind==='threeBet')return'raise';
  if(spot.kind==='vs3bet'||spot.kind==='vs4bet')return'call';
  return'call';
}
function preflopDrillAcceptedActions(spot,chart){
  const primary=preflopDrillPrimaryAction(spot,chart);
  if(!chart||chart.status==='out')return['fold'];
  if(chart.status==='mix'){
    if(spot.kind==='vs3bet'||spot.kind==='vs4bet')return['fold','call','raise'];
    return Array.from(new Set([primary,'fold']));
  }
  if(spot.kind==='vs3bet'||spot.kind==='vs4bet')return['call','raise'];
  return[primary];
}
function preflopDrillCategory(spot){
  if(!spot)return'preflop-entry';
...
function preflopDrillBuildSpot(opts){
  ...
  const pos=opts.pos||base.positions[Math.floor(Math.random()*base.positions.length)]||'BTN';
  const totalP=opts.totalP||6;
  ...
  const actionLine=base.type==='open'
    ?pos+'で未参加からアクション'
    :base.type==='flat'
      ?(base.openerPos||'CO')+'オープンに'+pos+'で対応'
      :base.type==='vs3bet'
...
function renderPreflopDrill(){
  ...
  if(!panel||!preflopDrillState)return;
  ...
  document.querySelectorAll('#pf-drill-actions button').forEach(function(b){b.disabled=false;b.classList.remove('selected-good','selected-bad');});
  const next=$('pf-drill-next');if(next)next.disabled=true;
}
function preflopDrillNextSpot(){
  if(!preflopDrillState)return;
  preflopDrillState.current=preflopDrillBuildSpot({stackBB:+($('cfg-stack')&&$('cfg-stack').value||100),totalP:+($('cfg-players')&&$('cfg-players').value||6)});
  renderPreflopDrill();
}
function startPreflopDrillSession(){
  _preflopDrillMode=true;_scenarioMode=false;
  document.body.classList.add('pf-drill-active');
  const panel=$('preflop-drill-panel');if(panel)panel.classList.remove('hidden');
  preflopDrillState={index:0,correct:0,answers:[],current:null,finished:false};
  const hudTitle=document.querySelector('#hud .hud-title');if(hudTitle)hudTitle.textContent='⚡ Fish Tank — プリフロップ即答';
  const hudInfo=$('hud-info');if(hudInfo)hudInfo.textContent='20問ドリル';
  refreshHudPracticeFocus();
  preflopDrillNextSpot();
}
function answerPreflopDrill(action){
  if(!preflopDrillState||preflopDrillState.finished||!preflopDrillState.current)return;
  const spot=preflopDrillState.current;
  const ev=preflopDrillEvaluateSpot(spot,action);
  if(ev.ok)preflopDrillState.correct++;
  preflopDrillState.answers.push({spot,action,result:ev});
  const result=$('pf-drill-result');
  if(result){result.className='pf-drill-result '+(ev.ok?'good':'bad');result.textContent=ev.text+' 推奨: '+ev.accepted.map(preflopDrillActionLabel).join(' / ');}
  document.querySelectorAll('#pf-drill-actions button').forEach(function(b){
    const a=b.dataset.pfAction;
    b.disabled=true;
    if(a===action)b.classList.add(ev.ok?'selected-good':'selected-bad');
  });
  preflopDrillState.index++;
  const next=$('pf-drill-next');
  if(next){next.disabled=false;next.textContent=preflopDrillState.index>=PREFLOP_DRILL_TOTAL?'結果を見る':'次の問題';}
}
function finishPreflopDrillSession(){
  if(!preflopDrillState)return;
  preflopDrillState.finished=true;
  const total=preflopDrillState.answers.length||1;
  const misses=preflopDrillState.answers.filter(function(a){return !a.result.ok;});
  misses.forEach(function(a,i){
    recordPrimaryLessonStats(sessionStats,{category:a.result.category,title:a.result.category==='ring-reraised-pot'?'3BET/スクイーズ対応を整理する':'フロップ前の入口を整理する',modeLabel:'プリフロップドリル',severity:'bad'},(sessionStats.hands||0)+i+1);
  });
  _saveStats();renderTrends();
  const byCat={};
  misses.forEach(function(a){byCat[a.result.category]=(byCat[a.result.category]||0)+1;});
  const weak=Object.keys(byCat).sort(function(a,b){return byCat[b]-byCat[a];})[0]||'なし';
  const result=$('pf-drill-result');
  if(result){result.className='pf-drill-result '+(misses.length?'bad':'good');result.textContent='結果: '+preflopDrillState.correct+' / '+total+' 正解。弱点: '+(weak==='ring-reraised-pot'?'3BET/スクイーズ対応':weak==='preflop-entry'?'フロップ前の入口':'なし')+'。傾向タブの弱点ランキングにも反映しました。';}
  const next=$('pf-drill-next');if(next)next.disabled=true;
}
...
document.addEventListener('input',function(e){
  var ta=e.target&&e.target.closest?e.target.closest('[data-op-note-name]'):null;
  if(!ta)return;
  var key=ta.getAttribute('data-op-note-name');
  setOpponentNote(key,ta.value);
  document.querySelectorAll('[data-op-note-saved="'+key+'"]').forEach(function(el){
    el.textContent=ta.value?'保存済み':'未記入';
  });
});
...
var _statsDef={
  ...
  steal:0, stealOpp:0,
  // 収支と判断品質の分離表示 (ROADMAP Phase 4-5)
  profitBB:0, profitHands:0, profitByHand:[],
...
function normalizeProfitStats(stats){
  stats=stats||{};
  stats.profitBB=Number(stats.profitBB)||0;
  stats.profitHands=Number(stats.profitHands)||0;
  stats.profitByHand=Array.isArray(stats.profitByHand)?stats.profitByHand.slice(-80):[];
  return stats;
}
function handProfitChips(hr){
  if(!hr||!hr.players)return 0;
  const human=hr.players.find(function(p){return p.isHuman;});
  if(!human)return 0;
  const invested=Number(human.totalInvested)||hr.decisions.filter(function(d){return d&&d.isHuman;}).reduce(function(s,d){return s+(Number(d.amount)||0);},0);
  const won=(hr.winners||[]).filter(function(w){return w&&w.player&&w.player.isHuman;}).reduce(function(s,w){return s+(Number(w.amount)||0);},0);
  return won-invested;
}
function handProfitBB(hr){
  const bb=(hr&&hr.bigBlind)||5;
  return bb?handProfitChips(hr)/bb:0;
}
function sessionStatsBb100(stats){
  stats=normalizeProfitStats(stats||sessionStats);
  return stats.profitHands>0?(stats.profitBB/stats.profitHands*100):0;
}
function renderQualityMoneyBand(stats,avgScore){
  stats=normalizeProfitStats(stats||sessionStats);
  avgScore=Number(avgScore)||0;
  const bb100=sessionStatsBb100(stats);
  const bb100Txt=(bb100>=0?'+':'')+(Math.round(bb100*10)/10)+' bb/100';
  const profitColor=bb100>=25?'var(--green)':bb100<=-25?'var(--red)':'var(--gold)';
  const qualityColor=avgScore>=80?'var(--green)':avgScore>=65?'var(--gold)':'var(--red)';
  let note='';
  if(stats.profitHands>=5&&avgScore>=78&&bb100<0){
    note='判断品質は悪くありません。短期の収支は分散に振れます。今はスコアを実力側の指標として見てください。';
  }else if(stats.profitHands>=5&&avgScore<65&&bb100>0){
    note='収支は勝っていますが、判断品質はまだ不安定です。短期の勝ちを実力と決めず、レビューのミスを優先して直しましょう。';
  }else{
    note='bb/100は100ハンドあたりの獲得BB数です。収支は短期分散、判断スコアは判断品質として分けて見ます。';
  }
  let html='<div style="display:grid;grid-template-columns:1fr 1fr;gap:4px;margin-bottom:6px">';
  html+='<div style="background:var(--panel2);border-radius:7px;padding:6px 7px;border-left:2px solid '+qualityColor+'"><div style="font-size:9px;color:var(--dim)">判断品質</div><div style="font-size:15px;font-weight:900;color:'+qualityColor+'">'+((stats.scores||[]).length?avgScore+'pt':'--')+'</div></div>';
  html+='<div style="background:var(--panel2);border-radius:7px;padding:6px 7px;border-left:2px solid '+profitColor+'"><div style="font-size:9px;color:var(--dim)">実収支</div><div style="font-size:15px;font-weight:900;color:'+profitColor+'">'+(stats.profitHands?bb100Txt:'--')+'</div></div>';
  html+='</div><div style="font-size:10px;color:var(--dim);line-height:1.5;background:var(--panel2);border-radius:7px;padding:6px 8px;margin-bottom:8px">'+note+'</div>';
  return html;
}
...
function updateSessionStats(hr,an){
  ...
  const pbb=handProfitBB(hr);
  sessionStats.profitBB=(Number(sessionStats.profitBB)||0)+pbb;
  sessionStats.profitHands=(Number(sessionStats.profitHands)||0)+1;
  sessionStats.profitByHand=(sessionStats.profitByHand||[]).concat(Math.round(pbb*10)/10).slice(-80);
...
function renderTrends(){
  ...
  scoreBand+='</div>';
  var qualityMoneyBand=renderQualityMoneyBand(sessionStats,avgScore);
  // 3BET% / Steal% / Limp%
  ...
  var finalHTML=sampleNote+qualityMoneyBand+scoreBand+sparkline+g1+g2+g3+lessonWeaknessHtml+leakHtml;
...
function startNewHand(){
  ...
  game.waitingForHuman=false;game._lastWinners=[];game._handEnded=false;
  game.players.forEach(p=>p._rebought=false);
  if(_preflopDrillMode){startPreflopDrillSession();return;}
...
function _initGame(mode){
  ...
  _preflopDrillMode=(mode==='preflop_drill');
  document.body.classList.toggle('pf-drill-active',_preflopDrillMode);
  const drillPanel=$('preflop-drill-panel');if(drillPanel)drillPanel.classList.toggle('hidden',!_preflopDrillMode);
  showScreen('game-screen');
  // HUDにモード表示
  const hudTitle=document.querySelector('#hud .hud-title');
  if(hudTitle)hudTitle.textContent=_preflopDrillMode?'⚡ Fish Tank — プリフロップ即答':(_scenarioMode?'🎯 Fish Tank — フロップトレーナー':(mode==='tournament'?'🏆 Fish Tank — Tournament':'🐟 Fish Tank Poker'));
  refreshHudPracticeFocus();
  startNewHand();
}
...
document.querySelectorAll('#pf-drill-actions button').forEach(function(btn){
  btn.addEventListener('click',function(){answerPreflopDrill(btn.dataset.pfAction);});
});
const pfDrillNext=$('pf-drill-next');
if(pfDrillNext){
  pfDrillNext.addEventListener('click',function(){
    if(!preflopDrillState)return;
    if(preflopDrillState.index>=PREFLOP_DRILL_TOTAL){finishPreflopDrillSession();return;}
    preflopDrillNextSpot();
  });
}
const pfDrillFinish=$('pf-drill-finish');
if(pfDrillFinish){
  pfDrillFinish.addEventListener('click',function(){finishPreflopDrillSession();});
}
...
function openSheet(tab){
  ...
  ['history','trends','glossary','live','notes'].forEach(function(id){
  ...
  if(tab==='notes'){var sn=$('stab-notes');if(sn)sn.innerHTML=renderOpponentNotesPanel();}
...
document.querySelectorAll('[data-tab]').forEach(function(btn){
  btn.addEventListener('click',function(){
    ...
    ['history','trends','glossary','live','notes'].forEach(function(id){var el=$('tab-'+id);if(el)el.classList.toggle('hidden',id!==t);});
    ...
    if(t==='notes'){var tn=$('tab-notes');if(tn)tn.innerHTML=renderOpponentNotesPanel();}
...
document.addEventListener('click',function(e){
  ...
  const startPracticeBtn=e.target&&e.target.closest&&e.target.closest('.session-start-practice');
  if(startPracticeBtn){
    startSessionPracticeRecommendation({
      modeValue:startPracticeBtn.dataset.mode||'normal',
      focusValue:startPracticeBtn.dataset.focus||'',
      presetValue:startPracticeBtn.dataset.preset||'',
      mode:startPracticeBtn.dataset.modeLabel||'',
      focus:startPracticeBtn.dataset.focusLabel||'',
      status:startPracticeBtn.dataset.status||'',
      reason:startPracticeBtn.dataset.reason||''
    });
    return;
  }
  ...
  if(e.target&&(e.target.id==='btn-reset-stats'||e.target.id==='btn-reset-stats2')){
    if(confirm('統計データをリセットしますか？（累積ハンド数・VPIP・CBet等がすべてクリアされます）')){
      ...
      sessionStats.profitBB=0;sessionStats.profitHands=0;sessionStats.profitByHand=[];
