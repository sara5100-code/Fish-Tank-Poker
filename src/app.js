...
function sessionPracticeRecommendation(focus){
  ...
  if(/BB防衛|BBディフェンス|defend/i.test(txt))return{mode:'トーナメント局面別',focus:'BBディフェンス練習',reason:'BB防衛はポットオッズだけでなく、OOPで払いすぎない感覚まで一緒に練習できます。'};
  if(/リバー|ワンペア|ショーダウン|WTSD/.test(txt))return{mode:'リングゲーム',focus:'リバー判断',reason:'ライブ$2/$5で差が出る、ワンペアの受けすぎと薄バリューの切り分けを重点的に見ます。'};
  if(/ポストフロップ|フロップ|ターン|PostF/.test(txt))return{mode:'フロップトレーニング',focus:'ポストフロップ判断',reason:'ボード、ポジション、ベット目的を短い反復で確認できます。'};
  if(/リンプ|入口|VPIP|参加|フロップ前/.test(txt))return{mode:'リングゲーム',focus:'フロップ前の入口整理',reason:'参加する手と降りる手を整理し、難しいワンペア判断を最初から減らします。'};
  if(/ミス|テーマ|振り返り/.test(txt))return{mode:'リングゲーム',focus:'主テーマ確認',reason:'まずは通常ハンドで、レビューの主テーマを一つずつ潰していきます。'};
  return null;
}
function renderSessionPracticeRecommendation(focus){
  const rec=sessionPracticeRecommendation(focus);
  if(!rec)return '';
  return '<div class="session-recommend"><div class="session-recommend-title">おすすめ練習: '+sessionTextHTML(rec.mode)+' / '+sessionTextHTML(rec.focus)+'</div><div class="session-recommend-body">'+sessionTextHTML(rec.reason)+'</div></div>';
}
function renderSessionCarryOver(focus){
  if(!focus)return '';
  const color=focus.tone==='good'?'var(--green)':focus.tone==='warn'?'var(--orange)':'var(--accent)';
  return '<div class="session-summary session-carryover" style="border-left-color:'+color+'"><div class="session-summary-title">前回からの引き継ぎ: '+sessionTextHTML(focus.title.replace(/^次回の一点:\s*/,''))+'</div><div class="session-summary-body">'+sessionTextHTML(focus.body)+'</div>'+renderSessionPracticeRecommendation(focus)+'</div>';
}
